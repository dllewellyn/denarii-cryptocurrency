...
class CalculateMaxTradeTest {
    ...
    fun `calculate max trade example one`() {
        ...
        val trade = CalculateMaxTrade { account }.calculateMaxTrade(BigDecimal.fromInt((100)))
        assertEquals(BigDecimal.fromInt(1), trade)
    }
    ...
    fun `calculate max trade example four`() {
        ...
        assertEquals(
            BigDecimal.fromDouble(1.5),
            CalculateMaxTrade { account }.calculateMaxTrade(BigDecimal.fromInt(150))
        )
    }
}